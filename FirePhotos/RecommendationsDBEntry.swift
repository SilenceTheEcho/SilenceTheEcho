...
class RecommendationsDBEntry: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func handleSignUp(){
        ...
        Database.database().reference().child("Recomendations").updateChildValues(valuesToSend, withCompletionBlock: { (err, ref) in
